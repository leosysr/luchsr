...
CheckMK-Tray-Client für Windows. Schlanke Alternative zu Nagstamon, bewusst nur CheckMK,
kein Multi-Backend. Diese Datei ist der Einstiegspunkt für jede neue Sitzung.
...
│   ├── make-sounds.mjs        DIE HINWEISTÖNE: Synthese, WAV, Selbstprüfung
...
└── src-tauri/                 Backend
    ...
    ├── sounds/                ERZEUGT — scripts/make-sounds.mjs, ins Binary eingebaut
    └── src/
...
- `msiexec /i Luchsr-1.0.0-x64.msi /qn` muss silent durchlaufen
...
| D53 | Lizenz ist **MIT**. `THIRD-PARTY.md` wird **erzeugt**, nicht gepflegt | MIT, weil das Werkzeug ohne Rechtsprüfung weitergegeben werden soll; von Nagstamon (GPL-2.0) ist keine Zeile übernommen, es besteht also keine Copyleft-Pflicht. Die Fremdliste zu pflegen hiesse, sie beim nächsten `cargo update` falsch zu haben — `scripts/third-party.ps1` liest `cargo tree` für **nur das Windows-Ziel** und die npm-Laufzeitabhängigkeiten. Cargo.lock erfasst alle Plattformen; Crates zu nennen, die im Build nicht landen, wäre Lärm statt Auskunft |
...
| D63 | Die Hinweistöne sind **erzeugt** (`scripts/make-sounds.mjs`), nicht besorgt: Sinus, weiche Hüllkurve, zwei bis vier Töne, **unter 350 ms** | Dieselbe Begründung wie bei der Bildmarke: eine heruntergeladene Datei hat eine Herkunft und eine Lizenz, die mitzuführen sind — ein Skript, das sie erzeugt, hat beides nicht. Kurz, weil die Klänge in `C:\Windows\Media` halbe bis ganze Sekunden lang sind: wer zwanzig Meldungen am Tag bekommt, hört sie zwanzigmal und schaltet sie ab. Sinus statt Rechteck, weil Obertöne über Laptoplautsprecher schrill klingen. Das Skript **liest die erzeugten Dateien zurück** und prüft Kopf, Länge und Spitzenwert — ein falscher WAV-Kopf ergibt sonst Stille ohne Fehlermeldung |
...
8. ✅ **Benachrichtigungen** — Toasts bei Änderung, Entwarnungen, Deckelung, sechs erzeugte Hinweistöne mit Auswahl je Ereignis. 368 Rust-Tests, 47 Frontend-Tests
